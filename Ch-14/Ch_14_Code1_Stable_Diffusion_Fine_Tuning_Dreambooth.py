...
instance_prompt = "<cat-toy> toy" #specific concept we are teaching.
prior_preservation = False
prior_preservation_class_prompt = "a photo of a cat clay toy"
num_class_images = 12 #If prior-preservation were enabled, this would specify how many class images (e.g., generic cat clay toy images) should be generated by the model itself to help it retain knowledge of the broader class.
sample_batch_size = 2
prior_loss_weight = 0.5 #This is only relevant if prior_preservation is True (weightage for generalization)
prior_preservation_class_folder = "./class_images" #This defines the directory where the generated class images (if prior-preservation is used) would be stored.
...
class DreamBoothDataset(Dataset):
    def __init__(
        ...
        if class_data_root is not None:
            ...
            list(Path(class_data_root).iterdir())
            self.num_class_images = len(self.class_images_path)
            self._length = max(self.num_class_images, 
...
if(prior_preservation):
    ...
    if cur_class_images < num_class_images:
        pipeline = StableDiffusionPipeline.from_pretrained(
            pretrained_model_name_or_path, revision="fp16",  
        ...
        for example in tqdm(sample_dataloader, desc="Generating class 
        images"):
            images = pipeline(example["prompt"]).images
            for i, image in enumerate(images):
                image.save(class_images_dir / f"{example['index'][i] + 
                cur_class_images}.jpg")
        pipeline = None
        gc.collect()
        del pipeline
        with torch.no_grad():
...
def training_function(text_encoder, vae, unet):
    ...
    accelerator.num_processes > 1:
        raise ValueError(
            "Gradient accumulation is not supported when training the text encoder 
    ...
    params_to_optimize = (
        itertools.chain(unet.parameters(), text_encoder.parameters()) if 
        args.train_text_encoder else unet.parameters()
    )
    ...
    train_dataset = DreamBoothDataset(
        ...
        class_data_root=args.class_data_dir if args.with_prior_preservation else 
        None,
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                to(dtype=weight_dtype)).latent_dist.sample()
                ...
                if args.with_prior_preservation:
                    ...
                    noise_pred, noise_pred_prior = torch.chunk(noise_pred, 2, 
                    dim=0)
